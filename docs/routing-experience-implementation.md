...
后台反馈分析最多 10 次分析尝试，首次分析、跨服务商自动切换和人工重试共用额度；OpenAI/DeepSeek API 不可用时仅尝试允许列表中另一已配置服务商的一个模型，输出解析失败不自动切换。无待处理任务时 worker 退出。前端轮询和导出分页也有限额。选择强模型可能增加调用费用，该选项不默认开启。详见[反馈收件箱与故障切换](feedback-layer.md#反馈收件箱与-api-故障切换2026-09-18)。
