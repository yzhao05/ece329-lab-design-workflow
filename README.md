...
- `GUIDED_DESIGN` 与 `EMVR_DIRECT` 统一优先处理用户的课内要求。提问、参考、总结、检查或未明确片段所在的回合暂停阶段推进，完成按钮也不能跳过这些请求；明确修改仍先保存，同轮多个服务请求分别回应。处理后从原进度继续，参考仍须用户采纳；
- 课内提问可涉及其他阶段，生成层对提问和参考使用独立回复契约，不再要求附带当前阶段的整套产物，也不把回答重写成阶段一大纲；课程引用校验继续生效。解释暂不可用时保留问题，不以阶段模板冒充回答；
...
EMVR Builder PDF 的本机目录定位、包内边界、内嵌资料和 Value 完整性规则见 [Builder PDF 可移植性说明](docs/builder-pdf-portability.md)。旧会话不再需要提供机器绝对路径；emvr33 的修订审阅件可用文档中的导出脚本复现。
...
阶段 3 单步查看误覆盖完整流程、阶段 7 无法补填、磁场参考与定性报告矛盾的修复，以及旧会话恢复方法，见 [emvr35 维护记录](docs/emvr35-recovery.md)。

两个模式均支持询问当前待答问题、进度、修改记录和作答方式；这些元问题保留原待办和设计状态，详见 [对话元问题说明](docs/meta-questions.md)。

混合元问题、否定操作、跨实验历史恢复、Common 接口和有限计算预算的后续修复，以及闭合圆环磁场 PDF 与 CMD 推送方法，见 [emvr35 项目复核](docs/emvr35-project-review.md)。

多项请求遗漏、跨阶段栏目误绑定、抽象意见误写入及重复确认的修复，见 [emvr36 对话维护记录](docs/emvr36-dialogue-recovery.md)。
...
点击输入框右下角的当前模型名称，可展开绿色设置浮窗，选择 Recommended / Fast / High Quality / Custom 策略、逐阶段能力覆盖和具体模型。后端通过 `ECE329_ALLOWED_MODELS` 控制 OpenAI 与 DeepSeek 的可选范围，通过 `ECE329_MODEL_REGISTRY` 和 `ECE329_STAGE_POLICY` 控制路由。旧会话固定模型选择保持兼容，见 [模型选择说明](docs/model-selection.md)。顶部语言按钮支持中英文界面、回复与导出，见 [语言与模型浮窗说明](docs/language-and-model-popover.md)。
...
思考强度、累计预算、双语缓存循环与译文引用保护的增补审阅及 CMD 推送命令见 [本轮审阅记录](docs/review-reasoning-language-2026-09-12.md)。
