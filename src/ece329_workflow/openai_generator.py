    ...
    def _resolve_intent_impl(
        ...
        payload = {
            ...
            "instructions": (
                ...
                "这些页面名称跨阶段保持同一含义，不能把‘之前的可用交互’改写成当前阶段的交互目标。"
                "若用户要求把A栏内容移到B栏并替换A，先读取修改前的A，再输出B和A两个动作；不得只执行最后一个。"
                "‘还需要明确比较关系’等反馈是改写要求，不能原样保存成研究问题；结合现有变量给出具体改写，"
                "无法确定时保留原值并仅澄清该项。不要把多个待答字段包装成一个笼统问题。"
                "EMVR栏目要分别绑定：主要课程主题用MODIFY_EMVR_FIELD/primary_course_concept_id（lecture_12等目录ID），"
                "课程讲义依据用course_reference_ids（讲次ID列表）；课程关系仍是独立的course_relationship说明，不得因修改主题而改写。"
                "核心公式用primary_formula_ids，辅助/支撑公式用supporting_formula_ids，值为已检索的公式ID列表；"
                "移动公式必须同时更新两个角色，不得把‘删除第三条’或公式列表写入theoretical_framework。"
                "用户指出两栏目不一致时，利用已选公式和目录定位矛盾并修正，不要删除栏目或要求重述已知问题。"
                "路径形状选项用path_shape_options，和研究对象、变化量分别保存。用户说参考无关时先核对待补字段，重新提供对应参考。"
                ...
                "它是学生选择公式时同时给出的实验重点；后续选择实验方法或整理方向时必须继续纳入，不能"
                "只保留公式名称而遗漏其中的对象、变化量或观察意图。实验方向锁定前，学生明确要求更换、删除或"
