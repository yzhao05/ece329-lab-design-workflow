...
(async () => {
  ...
  try {
    ...
    await admin.locator('#reviewStatus').filter({hasText:'请先填写人工修改后的正确内容'}).waitFor();
    assert.equal(await card.locator('textarea[id^=original-]').getAttribute('readonly'), '');
    const correctedSummary = '上一轮已邀请用户确认继续时，应承接确认并推进对应事项；真实阻塞需说明。';
    await card.locator("textarea[id^=corrected-]").fill(correctedSummary);
    await card.locator('select[id^=correction-field-]').selectOption('trigger');
    assert.equal(await card.locator('textarea[id^=original-]').inputValue(), candidate.content.trigger);
    assert.equal(await card.locator('textarea[id^=corrected-]').getAttribute('maxlength'), '140');
    const correctedTrigger = '上一轮明确邀请确认继续，用户随后回复继续下一步。';
    await card.locator('textarea[id^=corrected-]').fill(correctedTrigger);
    await card.locator('select[id^=correction-field-]').selectOption('summary');
    assert.equal(await card.locator('textarea[id^=corrected-]').inputValue(), correctedSummary);
    ...
    assert.equal(JSON.parse(enabled.reviews[0].note).original, candidate.content.summary);
    await admin.locator('#languageToggle').click();
    await card.getByText('Human review: select the field to correct', {exact:true}).waitFor();
    await admin.locator('#languageToggle').click();
    await card.getByText('人工审阅：选择需要修正的字段', {exact:true}).waitFor();
